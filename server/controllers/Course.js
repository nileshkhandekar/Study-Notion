...
exports.createCourse = async (req, res) => {
  try {
    ...
    console.log("tag and instructions parsed");
    console.log("tag", tag)
  ...
  try {
    ...
    for (var i = 0; i < courses.length; i++) {
      ...
      for (var j = 0; j < courses[i].courseContent.length; j++) {
        totalDurationInSeconds += courses[i].courseContent[
          j
        ].subSection.reduce((acc, curr) => acc + parseInt(curr.timeDuration), 0)
      }
      let totalDuration = convertSecondsToDuration(totalDurationInSeconds)
      // console.log(`Duration of Course ${i} = ${totalDuration}`);
      duration[i] = totalDuration;
    }

    // console.log("with Duration... Courses : ", courses)
    // console.log("Duration array :", duration);
    // Return the instructor's courses
    res.status(200).json({
      success: true,
      data: instructorCourses, duration
    })
  } catch (error) {
    console.error(error)
    res.status(500).json({
